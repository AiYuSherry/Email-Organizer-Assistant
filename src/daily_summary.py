...
def get_today_emails():
    """获取今天（00:00 到现在）的所有邮件"""
    log_message("📥 正在连接 QQ 邮箱，获取今日邮件...")
    ...
    try:
        ...
        today_str_imap = now.strftime('%d-%b-%Y')
        log_message(f"   搜索今天 ({today_str_imap}) 的邮件...")
        
        _, messages = mail.search(None, f'(ON "{today_str_imap}")')
        msg_ids = messages[0].split()
        log_message(f"   找到 {len(msg_ids)} 封今日邮件")
        
        # 限制处理数量，但优先获取有日期的邮件
        max_emails = 30
        check_limit = min(len(msg_ids), 100)  # 检查最新的100封
        msg_ids_to_check = msg_ids[-check_limit:]
        
        log_message(f"   从最新 {check_limit} 封邮件中筛选昨天的邮件...")
        ...
        for i, num in enumerate(msg_ids_to_check):
            ...
            if (i + 1) % 10 == 0 or i == 0:
                log_message(f"   检查中... {i+1}/{len(msg_ids_to_check)} (已找到 {len(email_list)} 封)")
            ...
            try:
                ...
                # 只保留今天的邮件
                if email_date and email_date >= today_start:
        ...
        log_message(f"✅ 获取到 {len(email_list)} 封今日邮件")
        return email_list
...
<body>
    <div class="header">
        <h1>📧 今日邮件日报</h1>
        <p>{today_str} | 共处理 {len(emails)} 封邮件 | 🔴需处理 {total_action} 封 | ⚪参考 {total_ref} 封</p>
    </div>
'''
    ...
    if total_action == 0 and total_ref == 0:
        ...
        <div style="text-align:center;padding:60px 20px;color:#666;">
            ...
            <div style="font-size:18px;">今天没有收到新邮件</div>
    ...
    html += f'''
    <div class="footer">
        <p>📧 此邮件由邮件助手自动生成 | ⏰ 发送时间：{datetime.now().strftime('%H:%M')}</p>
...
def send_email(html_content, email_count):
    ...
    try:
        ...
        msg['From'] = QQ_EMAIL
        msg['To'] = QQ_EMAIL
        msg['Subject'] = f"📧 今日邮件日报 - {today_str}（{email_count}封）"
